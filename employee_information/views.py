...
from employee_information.models import Department, Position, Employees, Registration, Tools
...
def registration(request):
    ...
    return render(request, 'employee_information/registration.html',context)
...
def tools(request):
    position_list = Tools.objects.all()
    context = {
        'page_title':'Tools',
        'toolss':position_list,
    }
    return render(request, 'employee_information/tools.html',context)
@login_required
def manage_tools(request):
    ...
    if request.method == 'GET':
        ...
        if 'id' in data:
            id= data['id']
        if id.isnumeric() and int(id) > 0:
            tool = Tools.objects.filter(id=id).first()
    
    context = {
        'tools' : tool
    }
    return render(request, 'employee_information/manage_tools.html',context)
...
def save_tools(request):
    ...
    try:
        if (data['id']).isnumeric() and int(data['id']) > 0 :
            save_tools = Tools.objects.filter(id = data['id']).update(name=data['name'], description = data['description'], status = data['status'])
        else:
            save_tools = Tools(name=data['name'], description = data['description'],status = data['status'])
            save_tools.save()
        resp['status'] = 'success'
    except:
        resp['status'] = 'failed'
...
def delete_tools(request):
    ...
    resp = {'status':''}
    try:
        Tools.objects.filter(id = data['id']).delete()
        resp['status'] = 'success'
    except:
        resp['status'] = 'failed'
    return HttpResponse(json.dumps(resp), content_type="application/json")
